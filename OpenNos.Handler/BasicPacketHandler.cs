...
namespace OpenNos.Handler
{
    public class BasicPacketHandler : IPacketHandler
    {
        ...
        public void Whisper(string packet)
        {
            ...
            bool? whisperBlocked = ServerManager.Instance.GetProperty<bool?>(packetsplit[(packetsplit[1] == "/GM" ? 3 : 2].Substring(1), nameof(Character.WhisperBlocked));
            if (whisperBlocked.HasValue)
            {
                if (!whisperBlocked.Value || Session.Account.Authority == AuthorityType.Admin)
                {
                    ServerManager.Instance.Broadcast(Session, Session.Character.GenerateSpk(message, (Session.Account.Authority == AuthorityType.Admin ? 15 : 5)), ReceiverType.OnlySomeone, packetsplit[(packetsplit[1] == "GM" ? 3 : 2].Substring(1));
                }
                else
                {
